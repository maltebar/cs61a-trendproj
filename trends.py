...
def extract_words(text):
    ...
    for each_letter in text:
        if each_letter in ascii_letters:
            new_word = new_word + each_letter
        else:
            if new_word != '':
                word_list =  word_list + [str(new_word)]
                new_word = ''
    if (text[len(text)-1] in ascii_letters):
        word_list =  word_list + [str(new_word)]
    return word_list
...
def analyze_tweet_sentiment(tweet):
    ...
    total_sum = 0
    number_of_words_with_sentiment = 0
    "*** YOUR CODE HERE ***"
    for word in tweet_words(tweet):
        if has_sentiment(get_word_sentiment(word)):
            total_sum = total_sum + sentiment_value(get_word_sentiment(word))
            number_of_words_with_sentiment = number_of_words_with_sentiment + 1
    if number_of_words_with_sentiment == 0:
        return make_sentiment(None)
    else:
        return make_sentiment(total_sum / number_of_words_with_sentiment)
